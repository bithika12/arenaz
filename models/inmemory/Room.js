...
const  Dart            = require('../gameplay/Dart');
        ...
        room.findOne({roomName: reqObj.roomName}
            , function (err, result) {
                if (result) {
                    ...
                    resolve({
                        roomName: reqObj.roomName, users: reqObj.userId, remainingScore: calculatedScore,
                    ...
                    /*userArr.findIndex(function (elemt) {
                        ...
                        if (elemt.userId == reqObj.userId)
                            newArr.push({score:reqObj.score,total:calculatedScore,userId:elemt.userId,status:elemt.status,turn:userTurn,dartPoint:dartPnt,isWin:isWin});
                        else
                            newArr1.push({score:elemt.score,total:elemt.total,userId:elemt.userId,status:elemt.status,turn:elemt.turn,dartPoint:elemt.dartPoint,isWin:elemt.isWin});
        ...
        room.findOne({roomName: condObj.roomName}, function (err, roomDetails) {
            if (roomDetails) {
                ...
                if (findIndex == -1) {
                    ...
                    if (findIndex1 != -1)
                        resolve({userId: users[findIndex1].userId, userType: users[findIndex1].userType});
                    else
                        reject({message: "User not found"});
                } else {
                    resolve({userId: users[findIndex].userId, userType: users[findIndex].userType});
                }
            } else {
                reject({});
            }
...
/**
 * @desc This function is used for create username
 * @param {Number} length
 */
...
/**
 * @desc This function is used for insert AI details
 * @param {String} roomObj
 * @param {String} userObj
 */

room.insertAI = function (roomObj, userObj) {
    return new Promise((resolve, reject) => {
        room.findOne(roomObj, function (err, result) {
            if (err) reject({})
            else {
                let users = result.users
                if (users.length < constants.GAME_MAXIMUM_USER) {
                    //let userDirection = findAvailabePosition(users);

                    let colors = ['aqua', 'black', 'blue', 'fuchsia', 'gray', 'green',
                        'lime', 'maroon', 'navy', 'olive', 'orange', 'purple', 'red',
                        'silver', 'teal', 'white', 'yellow'];
                    let colorName = getRandomColor(colors);
                    let AIObj = {
                        "userId": userObj.userId, userType: "ai", userName: createUserName(4),
                        score: 100, total: 100, status: "active", isWin: 0, turn: 0, dartPoint: "",
                        colorName: colorName, raceName: colorName, total_no_win: 0
                        /*direction : userDirection*/
                    };
                    users.push(AIObj);
                    room.update({_id: roomObj._id}, {$set: {users: users}}, function (err, updateroomresult) {
                        resolve({"AI": true, AIObj: AIObj});
                    })
                } else resolve({"AI": false, AIObj: {}});
            }
        });
    })
}
/**
 * @desc This function is used for robot dart throw
 * @param {String} roomName
 * @param {String} userId
 */

room.robotThrow = function (roomName, userId) {
    return new Promise((resolve, reject) => {
        room.findOne({_id: roomName}, function (err, result) {
            if (result) {
                ...
                let bid = new Bid();
                if (listOfBid.length === 0) {
                    resolve({bid: bid.openerBid(cardSuits)})
                } else {
