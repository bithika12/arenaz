...
const { fetchNotification,userValidChk,changeNotificationStatus,fetchNotificationUnreadCount} = require(appRoot +'/models/FetchNotification');
...
exports.fetchGame= function(req,res) {
    ...
    if (!valid) {
        ...
        userValidChk(req.body.userEmail)
            .then(validResponse => {
                return fetchNotification(validResponse);
            })
...
exports.changeStatus= function(req,res) {
    ...
    if (!valid) {
        ...
        userValidChk(req.body.userEmail)
            .then(validResponse => {
                console.log("req.body.notificationId"+req.body.notificationId);
...
//////
exports.fetchUnreadMessage= function(req,res) {
    /*
      * Joi is used for validation
     */
    let schema = Joi.object().keys({
    ...
    if (!valid) {
        ...
        userValidChk(req.body.userEmail)
            .then(validResponse => {
                return fetchNotificationUnreadCount(validResponse);
            })
