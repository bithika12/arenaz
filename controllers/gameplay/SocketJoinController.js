	...
	socket.use((packet, next) => {
    	...
    	user.checkUserTokenMod({accessToken: packet[1].accessToken}).then((userDetails)=>{
    	    if (userDetails) {
				...
				packet[1].firstName = (!userDetails.firstName)?"":userDetails.firstName;
				packet[1].lastName = (!userDetails.lastName)?"":userDetails.lastName;
				...
				packet[1].cupNumbers = (!userDetails.cupNo)?0:userDetails.cupNo;
				//packet[1].cupNumbers = (!userDetails.cupNo)?"":userDetails.cupNo;
