...
angular.module('teenPatti.controllers').controller('gamePlay', ['$rootScope', '$filter', '$scope', 'cardService',
    function($rootScope, $filter, $scope, cardService) {
        ...
        if ($rootScope.userInfo) {
            ...
            socket.emit('adduser', {access_token :sessionStorage.getItem("access_token")});
            initSocketEvents();
        }
        $scope.currentPlayer = {};
        $scope.seatingInfo = {};
        ...
        function initSocketEvents() {
            ...
            */
            socket.on('user_connected', function(args) {
                console.log(args)
                $rootScope.userInfo.clientId = args.result.user_id;
                socket.emit('gamerequest',{access_token:sessionStorage.getItem('access_token'),code:'',game_id:1,amount:1000});
            });
            socket.on('connected_room', function(args) {
                $scope.seatingInfo[args.slot] = "currentPlayer";
                $scope.seatingInfoById[args.id] = "currentPlayer";
                $scope.currentPlayer           = args;
                console.log(" connected_room ",args);
    ...
    function($rootScope, $scope, userService) {
        ...
        $scope.register = function() {
            userService.register({
                ...
                if (res.status == '1') {
                    $rootScope.userInfo={login:true};
                    $rootScope.userInfo = res.result.userdetails;
                    sessionStorage.setItem("is_login", 1);
                    sessionStorage.setItem("access_token", res.result.access_token);
                    sessionStorage.setItem("user_info", $rootScope.userInfo);                    
